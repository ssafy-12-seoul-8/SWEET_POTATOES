...
def btk(cur, ry, rx, by, bx):           # 이번에 움직일 횟수, 빨간공좌표, 파란공 좌표
    global ans
    ...
    for k in range(4):
        ...
        elif sry == sby and srx == sbx: # 둘다 안들어갔는데 좌표가 겹친다는 것은 같은 행에 있었는데 가로 이동을 하거나 같은 열에 있었는데 세로 이동을 한 것
        ...
        if (ry, rx) == (sry, srx) and (by, bx) == (sby, sbx):   # 근데 좌표가 안바꼈다면 이 방향으로는 안가봐도 됨
            continue
        btk(cur + 1, sry, srx, sby, sbx)                        # 그렇지 않으면 함 가보자
